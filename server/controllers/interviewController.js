...
import { askAI } from '../services/openRouter.sevice.js';
...
export const generateQuestion = async (req,res)=>{
    try{
        ...
        if(user.credits < 50) return res.status(400).json({message:'Not enough creddits. Minimum 50 required'});
        ...
        if(!userPrompt.trim()) return res.status(400).json({message:'Prompt Content is empty.'});
...
`
    ...
    const aiResponse = await askAI(messages);
    if(!aiResponse || !aiResponse.trim()) return res.status(500).json({message:'AI returned empty response.'});

    const questionArray = aiResponse.split('\n').map(q => q.trim()).filter(q => q.length > 0).slice(0, 5);
    if(questionArray.length === 0) return res.status(500).json({message:'AI failed to generate question.'});
...
export const getMyInterviews = async (req,res)=>{
    try{
        ...
        return res.status(500).json({message:`failed to find interviews ${err}`})
    }
}
