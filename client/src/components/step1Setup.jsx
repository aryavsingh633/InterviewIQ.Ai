...
function Step1Setup({onStart}) {
  ...
  return (
    ...
    className = 'min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 px-4'>
          ...
          <div className = 'space-y-5'>
            {
              [
                {
                  icons: <FaUserTie className = 'text-green-600 text-xl'/>,
                  text:"Choose Role & Experience"
                },
                {
                  icons: <FaMicrophoneAlt className = 'text-green-600 text-xl'/>,
                  text:"Smart Voice Interview"
                },
                {
                  icons: <FaChartLine className = 'text-green-600 text-xl'/>,
                  text:"Performance Analytics"
                }
              ].map((item,index)=>(
                <motion.div initial = {{y:30,opacity:0}} animate = {{y:0,opacity:1}} transition = {{duration:0.3+index*0.15}}
        ...
        className = 'p-12 bg-white'>
          ...
          <div className = 'space-y-6'>
            ...
            <select value = {mode} onChange = {(e)=>setMode(e.target.value)}
              ...
              {!analysisDone && (
                <motion.div whileHover = {{scale:1.02}} onClick = {()=>document.getElementById("resumeUpload").click()}
                className = 'border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-green-500 hover:bg-green-50 transition'>
                  ...
                  {resumeFile && (
                    <motion.button whileHover = {{scale:1.02}} onClick = {(e)=>{e.stopPropagation(); handleUploadResume()}}
              ...
              disabled = {!role || !experience || loading} whileHover = {{scale:1.03}} whileTap = {{scale:0.95}}
