...
function Step3Report({ report }) {
  ...
  const downloadPDF = ()=>{
    ...
    if(finalScore >= 8){
      ...
      advice = "Good foundation shown. Improve clarity and structure. Practice delivery concise, confident answers with stronger supporting examples."
    }
    else{
    ...
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8'>
        <div className='space-y-6'>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <div className='relative w-20 h-20 sm:w-25 sm:h-25 mx-auto'>
              <CircularProgressbar value={percentage} text={`${score}/10`}
                styles={buildStyles({ textSize: "18px", pathColor: "#10b981", textColor: "#ef4444", trailColor: "#efe7eb" })} />
            </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className='bg-white rounded-xl sm:rounded-3xl shadow-lg p-6 sm:p-8'>
              ...
              {questionWiseScore.map((q,i)=>(
                <div key = {i} className = 'bg-gray-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-200'>
                  <div className = 'flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4'>
                    <div>
                      <p className = 'text-xs text-gray-00'>Question {i+1}</p>
