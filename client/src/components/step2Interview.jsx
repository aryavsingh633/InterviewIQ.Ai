...
function Step2Interview({interviewData, onFinish}) {
  ...
  useEffect(()=>{
    const loadVoices = ()=>{
      ...
      const femaleVoices = voices.find(v => v.name.toLowerCase().includes("zira") || 
      v.name.toLowerCase().includes("samantha") || v.name.toLowerCase().includes("female"));
      ...
      const maleVoices = voices.find(v => v.name.toLowerCase().includes("david") || 
      v.name.toLowerCase().includes("mark") || v.name.toLowerCase().includes("male"));
  ...
  useEffect(()=>{
    ...
    const runIntro = async ()=>{
      if(isIntroPhase){
        ...
        await speakText(`I will ask you few questions. Just answer naturally, and take your time. Let's begin.`);
    ...
    const submitAnswer = async ()=>{
      ...
      try{
        const result = await axios.post(ServerURL + "/api/interview/submit-answer", 
          {interviewId, questionIndex: currentIndex, answer, timeTaken: currentQuestion.timeLimit-timeLeft}, {withCredentials:true});
    ...
    <div className = 'min-h-screen bg-linear-to-br from-emerald-50 via-white to-teal-100 flex items-center justify-center p-4 sm:p-6'>
      <div className = 'w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden'>
        ...
        <div className = 'flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
          <h2 className = 'text-xl sm:text-2xl font-bold text-emerald-600 mb-5'>AI Smart Interview</h2>
          ...
          {!feedback ? (<div className = 'flex items-center gap-4 mt-6'>
            ...
            <motion.button whileTap = {{scale:0.95}} onClick = {submitAnswer} disabled = {isSubmitting}
